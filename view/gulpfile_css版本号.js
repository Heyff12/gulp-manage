...
var file_road = {
    ...
    jsNum_src:['./src/js/**/*.js','!./src/js/common/*.js', '!./src/js/plug/**/*.js', '!./src/js/require-config.js', '!./src/js/require-config2.js', '!./src/js/require.2.1.11.min.js'],
...
gulp.task('static', function() {
    gulp.start('css', 'images', 'js', 'reqconjs');
});
...
//#########################先执行 gulp del  清理文件,再执行gulp编译并监听,上传到服务器时，再执行一次 gulp html(增加上css版本号)
//开发构建--未执行['jshint'],
gulp.task('dev', function(done) {
    runSequence(
        ['images', 'css'], ['js', 'reqconjs'], ['html'], ['watch'],
        done);
});
